...
          {currentView === 'admin' && (user.role === 'Institution_Admin' || user.role === 'Platform_Admin') && <AdminView user={user} token={token} refreshUser={refreshUser} />}
        </div>
      </main>
    </div>
  );
...
const AdminView = ({ user, token, refreshUser }) => {
...
          <UserManagement user={user} token={token} refreshUser={refreshUser} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
const UserManagement = ({ user, token, refreshUser }) => { // FIX: Add refreshUser prop
...
      fetchPendingUsers();
      // FIX: Check if the approved user is the current logged-in user and refresh their state.
      if (user.id === userId) {
        refreshUser();
      }
    } catch (error) {
...
      fetchPendingUsers();
      // FIX: Check if the rejected user is the current logged-in user and refresh their state.
      if (user.id === userId) {
        refreshUser();
      }
    } catch (error) {
...
  // FIX: Add a function to refresh the user's data from the backend
...
      toast.success('Profile updated from server!'); // Optional: toast notification for debugging
    } catch (error) {
      console.error('Failed to refresh user data:', error);
      // Optional: Log user out if the token is invalid or expired
      handleLogout();
    }
  };
...
      // FIX: Immediately refresh the user data on load to ensure it's up-to-date
      refreshUser();
    }
  }, []);
...
        </>
      ) : (
        // FIX: Pass the new refreshUser function down to the Dashboard
