...
import { FaSun, FaMoon } from 'react-icons/fa'; // Import icons here
import './App.css';
...
import useDarkMode from './hooks/use-dark-mode'; // Import the custom hook
...
const InstitutionRegistrationModal = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg bg-white text-gray-950 border-gray-200 dark:bg-slate-900 dark:text-white dark:border-slate-700">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border-t border-gray-300 dark:border-slate-700 pt-4">
            <h4 className="text-lg font-semibold text-green-600 dark:text-green-400 mb-3">Institution Admin Details</h4>
          ...
          <Alert className="bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/50 dark:border-blue-700 dark:text-blue-200">
            <AlertDescription>
...
const AuthModal = ({ isOpen, onClose, mode, onToggleMode, onAuth, refreshUser }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white text-gray-950 border-gray-200 dark:bg-slate-900 dark:text-white dark:border-slate-700">
        <DialogHeader>
            ...
            <>
              ...
              <div>
                ...
                <Select value={formData.institution_id} onValueChange={(value) => setFormData({...formData, institution_id: value})}>
                  <SelectTrigger className="bg-gray-100 dark:bg-slate-800 border-gray-300 dark:border-slate-600">
...
const EmptyState = ({ icon, title, description, action }) => (
  <div className="text-center py-16">
    ...
    <h3 className="text-gray-950 dark:text-white text-xl font-semibold mb-2">{title}</h3>
...
const ErrorState = ({ title, description, onRetry }) => (
  <div className="text-center py-16">
    <div className="text-6xl mb-4 text-red-600 dark:text-red-400">⚠️</div>
    <h3 className="text-gray-950 dark:text-white text-xl font-semibold mb-2">{title}</h3>
    <p className="text-gray-600 dark:text-gray-400 mb-6">{description}</p>
    {onRetry && (
      <Button onClick={onRetry} variant="outline" className="border-red-500 text-red-600 dark:text-red-400 hover:bg-red-500/10">
        Try Again
      </Button>
    )}
  </div>
...
const ChatView = ({ token, chatId, user }) => {
    ...
    if (loading) return <div className="text-gray-950 dark:text-white text-center py-12">Loading chat...</div>;
    if (error) return <ErrorState title="Chat Error" description={error} onRetry={fetchChat} />;

    return (
        <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700 max-w-2xl mx-auto h-[600px] flex flex-col">
                        ...
                        <div
                            className={`rounded-lg p-3 max-w-[70%] text-sm ${
                                msg.sender_id === user.id
                                    ...
                                    : 'bg-gray-200 text-gray-950 dark:bg-slate-700 dark:text-gray-200'
                            }`}
                        >
                            <p>{msg.text}</p>
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 dark:border-slate-700">
                <div className="flex space-x-2">
                    <Input
                        ...
                        className="flex-1 bg-gray-100 text-gray-950 border-gray-300 dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                    />
...
const MentorRequestsView = ({ user, token, setCurrentView }) => {
  ...
  if (loading) return <div className="text-gray-950 dark:text-white text-center py-12">Loading requests...</div>;
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="text-gray-950 dark:text-white">Incoming Mentorship Requests</CardTitle>
        </CardHeader>
        <CardContent>
          {requests.length === 0 ? (
            <EmptyState
            ...
            <div className="space-y-4">
              {requests.map(req => (
                <div key={req.id} className="flex items-center justify-between p-4 bg-gray-200 rounded-lg dark:bg-slate-700">
                  <div>
                    <h4 className="text-gray-950 dark:text-white font-semibold">{req.student_name}</h4>
                    <p className="text-gray-600 dark:text-gray-400 text-sm">Requested mentorship</p>
                  </div>
                  <div className="flex space-x-2">
                    <Button 
...
const Dashboard = ({ user, token, onLogout, refreshUser }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-950 dark:bg-slate-900 dark:text-white transition-colors duration-300">
      ...
      <main className="pt-20 px-6">
        <div className="max-w-7xl mx-auto">
          {user.status === "Pending" && (
            <Alert className="mb-6 bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/50 dark:border-yellow-700 dark:text-yellow-200">
              <AlertDescription>
...
const Header = ({ user, onLogout, currentView, setCurrentView, chatsCount }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-b border-gray-200 dark:border-slate-700">
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-8">
            <h1 className="text-2xl font-bold text-gray-950 dark:text-white">Elevanaa</h1>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button variant="ghost" onClick={() => setCurrentView('profile')} className="text-gray-950 dark:text-white">
              Profile
            </Button>
            <Button variant="ghost" onClick={onLogout} className="text-gray-950 dark:text-white">
              Logout
            </Button>
          </div>
        </div>
...
const NavButton = ({ children, active, onClick }) => (
  <button
    ...
    className={`px-4 py-2 rounded-lg transition-all ${
      active 
        ? 'bg-blue-600 text-white' 
        : 'text-gray-600 dark:text-gray-300 hover:text-gray-950 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-slate-800'
    }`}
  >
    {children}
  </button>
...
const FeedView = ({ user, token }) => {
  ...
  if (loading) {
    return <div className="text-gray-950 dark:text-white text-center py-12">Loading feed...</div>;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {user.status === 'Verified' && (
        <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
          <CardHeader>
            <CardTitle className="text-gray-950 dark:text-white">Share an Update</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleCreatePost} className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-500 dark:text-gray-400">{newPost.length}/2000</span>
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700">
                  Share Post
                </Button>
              </div>
      ...
      {posts.length === 0 ? (
        ...
        posts.map((post) => (
          <Card key={post.id} className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <h4 className="font-semibold text-gray-950 dark:text-white">{post.author_name}</h4>
                    <span className="text-gray-500 dark:text-gray-400 text-sm">
                      {new Date(post.created_at).toLocaleDateString()}
                    </span>
                  </div>
                  <p className="text-gray-700 dark:text-gray-200 mb-4 whitespace-pre-wrap">{post.content}</p>
                  <div className="flex items-center space-x-4">
                    <Button
                      variant="ghost"
                      size="sm"
...
const DirectoryView = ({ user, token }) => {
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="text-gray-950 dark:text-white">Alumni Directory</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <Label className="text-gray-950 dark:text-white">Industry</Label>
              <Input
            ...
            <div>
              <Label className="text-gray-950 dark:text-white">Role</Label>
              <Select value={filters.role} onValueChange={(value) => setFilters({...filters, role: value})}>
                <SelectTrigger className="bg-gray-100 text-gray-950 border-gray-300 dark:bg-slate-700 dark:border-slate-600 dark:text-white">
      ...
      {loading ? (
        <div className="text-gray-950 dark:text-white text-center py-12">Loading directory...</div>
      ) : users.length === 0 ? (
        <EmptyState
          icon="👥"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {users.map((person) => (
            <Card key={person.id} className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700 card-hover">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-gray-950 dark:text-white font-semibold mb-2">
                  {person.first_name} {person.last_name}
                </h3>
                <div className="space-y-2">
                  <Badge className="bg-blue-600 text-white">{person.role}</Badge>
                  {person.major && (
                    <p className="text-gray-700 dark:text-gray-300 text-sm">{person.major}</p>
                  )}
                  {person.industry && (
                    <p className="text-gray-600 dark:text-gray-400 text-sm">{person.industry}</p>
                  )}
                  {person.location && (
                    <p className="text-gray-600 dark:text-gray-400 text-sm">📍 {person.location}</p>
                  )}
                  {person.is_mentor && (
...
const MentorView = ({ user, token, setCurrentView }) => {
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="text-gray-950 dark:text-white">AI-Powered Mentor Matching</CardTitle>
        </CardHeader>
      ...
      {matches.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {matches.map((mentor) => (
            <Card key={mentor.id} className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700 card-hover">
              <CardContent className="p-6 text-center">
                <Avatar className="w-16 h-16 mx-auto mb-4">
                ...
                <div className="space-y-2">
                  ...
                  {mentor.major && (
                    <p className="text-gray-700 dark:text-gray-300 text-sm">{mentor.major}</p>
                  )}
                  {mentor.industry && (
                    <p className="text-gray-600 dark:text-gray-400 text-sm">{mentor.industry}</p>
                  )}
                  {mentor.location && (
                    <p className="text-gray-600 dark:text-gray-400 text-sm">📍 {mentor.location}</p>
                  )}
                </div>
                <Button 
    ...
    <div className="space-y-6">
      <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-gray-950 dark:text-white">Job Board</CardTitle>
      ...
      {loading ? (
        <div className="text-gray-950 dark:text-white text-center py-12">Loading jobs...</div>
      ) : jobs.length === 0 ? (
        <EmptyState
          icon="💼"
        ...
        <div className="space-y-4">
          {jobs.map((job) => (
            <Card key={job.id} className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700 card-hover">
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h3 className="text-gray-950 dark:text-white font-semibold text-lg mb-2">{job.title}</h3>
                    <p className="text-blue-600 dark:text-blue-400 font-medium mb-2">{job.company}</p>
                    {job.location && (
                      <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">📍 {job.location}</p>
                    )}
                    <p className="text-gray-700 dark:text-gray-300 mb-4 whitespace-pre-wrap">{job.description}</p>
...
const ProfileView = ({ user, token, refreshUser }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="text-gray-950 dark:text-white">Profile Settings</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleUpdateProfile} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-gray-950 dark:text-white">Email</Label>
                <Input value={user.email} disabled className="bg-gray-100 text-gray-500 border-gray-300 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600" />
              </div>
              <div>
                <Label className="text-gray-950 dark:text-white">Major</Label>
            ...
            <div>
              <Label className="text-gray-950 dark:text-white">Location</Label>
              <Input
            ...
            {user.role === 'Alumni' && (
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <Label htmlFor="is_mentor" className="text-gray-950 dark:text-white">
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={currentTab} onValueChange={setCurrentTab}>
        <TabsList className="bg-gray-200 dark:bg-slate-800">
          {user.role === 'Platform_Admin' && (
            <TabsTrigger value="institutions" className="text-gray-950 dark:text-white">Institution Approvals</TabsTrigger>
          )}
          <TabsTrigger value="users" className="text-gray-950 dark:text-white">User Management</TabsTrigger>
        </TabsList>
...
const PlatformAdminInstitutions = ({ token }) => {
  ...
  if (loading) {
    return <div className="text-gray-950 dark:text-white text-center py-8">Loading pending institutions...</div>;
  }

  return (
    <Card className="bg-white border-gray-200 dark:bg-slate-800 dark:border-slate-700">
      <CardHeader>
        <CardTitle className="text-gray-950 dark:text-white">Pending Institution Approvals</CardTitle>
      </CardHeader>
      <CardContent>
        {pendingInstitutions.length === 0 ? (
          ...
          <div className="space-y-4">
            {pendingInstitutions.map((institution) => (
              <div key={institution.id} className="flex items-center justify-between p-4 bg-gray-200 rounded-lg dark:bg-slate-700">
                <div className="flex-1">
                  <h4 className="text-gray-950 dark:text-white font-semibold">{institution.name}</h4>
                  <p className="text-gray-700 dark:text-gray-300 text-sm">{institution.website}</p>
...
const UserManagement = ({ user, token }) => {
  ...
  if (loading) {
    return <div className="text-gray-950 dark:text-white text-center py-8">Loading pending users...</div>;
  }
        ...
        {pendingUsers.length === 0 ? (
          ...
          <div className="space-y-4">
            {pendingUsers.map((pendingUser) => (
              <div key={pendingUser.id} className="flex items-center justify-between p-4 bg-gray-200 rounded-lg dark:bg-slate-700">
                <div className="flex-1">
                  <h4 className="text-gray-950 dark:text-white font-semibold">
                    {pendingUser.first_name} {pendingUser.last_name}
                  </h4>
                  <p className="text-gray-700 dark:text-gray-300 text-sm">{pendingUser.email}</p>
...
const App = () => {
  ...
  const [isDarkMode] = useDarkMode(); // Initialize the hook to apply dark mode class
